...
def get_clique_mol(mol, atoms):
    ...
    new_mol = copy_mol(new_mol)
    new_mol = sanitize(new_mol)  # We assume this is not None
    return new_mol


def tree_decomp(mol):
    """ Decomposit the molecule into cliques, return the atom indices in each
    clique and the bonds connecting them
    """
    ...
    # list of neighboring cliques of each atom
...
def ring_bond_equal(b1, b2, reverse=False):
    """ Check if two bonds are equal by comparing the atoms at the ends of the
    bonds.
    """
...
def attach_mols(ctr_mol, neighbors, prev_nodes, nei_amap):
    """ Attach neighbors to the ctr_mol
    """
...
def enum_assemble(node, neighbors, prev_nodes=[], prev_amap=[]):
    ...
    for amap in all_attach_confs:
        ...
        if smiles in cand_smiles or \
                not check_singleton(cand_mol, node, neighbors):
            continue
        cand_smiles.add(smiles)
...
if __name__ == "__main__":
    ...
    smiles = [
        ...
        "C[C@H]1CC(=O)[C@H]2[C@@]3(O)C(=O)c4cccc(O)c4[C@@H]4O[C@@]43"
        "[C@@H](O)C[C@]2(O)C1",
    ...
    def decode_test():
        ...
        for tot, s in enumerate(smiles):
            ...
            cur_mol = copy_mol(tree.nodes[0].mol)
