...
class SamplerArguments(ArgumentParser):
    ...
    def __init__(self):
        super().__init__()
        self.add_argument("-s", "--source", required=True,
                          help="Path to the source file.")
        self.add_argument("-o", "--output", required=True,
                          help="Path to save the output.")
        self.add_argument("-n", "--num-samples", required=True, type=int,
                          help="Number of samples to sample from the source "
                               "file.")
        self.add_argument("-f", "--filter", action="store_true",
                          help="Filter similar SMILES.")
        self.add_argument("-v", "--verbos", action="store_true",
                          help="Show progress bar.")
        self.add_argument("-nh", "--no-header", action="store_false",
                          help="Add this tag if the source file does not have "
                               "header.")
        self.add_argument("-t", "--threshold", type=float, default=0.85,
